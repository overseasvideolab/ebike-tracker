...
# 1. Page Configuration (Set to wider layout for Dashboard feel)
st.set_page_config(layout="wide", page_title="Ebike Analytics Engine")

# 2. Advanced Styling (CSS)
st.markdown("""
<style>
    /* Widened max-width from 900px to 1200px to fit side-by-side dashboard columns */
    ...
    /* Styling for the new AI Habit Cards to make them look like Dashboard Widgets */
...
if raw_data:
    ...
    # Standardize Timezones to prevent crashes
    ...
    # ==========================================
    # DASHBOARD ROW 1 (Segments 2 & 3 side-by-side)
    # ==========================================
    ...
    with col_s2:
        ...
        if not full_month_history.empty:
            ...
            # FIX: textposition='auto' pushes text outside if the bar is too small to read!
            fig2.update_traces(marker_color='#F28C28', textposition='auto', texttemplate='<b>%{x:,.0f} km</b>', textfont_size=16)
            fig2.update_layout(height=280, margin=dict(l=0,r=40,t=10,b=0), plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', xaxis=dict(showgrid=False, visible=False))
            fig2.update_yaxes(type='category', title="", showgrid=False, tickfont=dict(size=14, weight='bold', color='#333'))
    ...
    with col_s3:
        ...
        # FIX: textposition='auto' for readable text
        fig3.update_traces(marker_color='#8e44ad', textposition='auto', texttemplate='<b>%{y:,.0f} km</b>', textfont_size=16)
        fig3.update_layout(height=280, margin=dict(l=0,r=0,t=10,b=0), plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', yaxis=dict(visible=False, showgrid=False))
        fig3.update_xaxes(type='category', title="", tickfont=dict(size=14, color='#333'))
        st.plotly_chart(fig3, use_container_width=True, config={'displayModeBar': False})

    st.write("---")

    # ==========================================
    # SEGMENT 4: Weather (Full Width)
    # ==========================================
    ...
    # ==========================================
    # DASHBOARD ROW 2 (Segments 5 & 6 side-by-side)
    # ==========================================
    col_s5, col_s6 = st.columns(2)
    
    with col_s5:
        st.write(f"### 📅 YEAR BY YEAR (UP TO {c_month_name.upper()} {c_day})")
        ...
        if not ytd_stats.empty:
            ...
            # FIX: textposition='auto'
            fig5.update_traces(marker_color='#4eb2e8', textposition='auto', texttemplate='<b>%{x:,.0f} km</b>', textfont_size=16)
            fig5.update_layout(height=260, margin=dict(l=0,r=20,t=10,b=0), plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', xaxis=dict(showgrid=False, visible=False))
            fig5.update_yaxes(type='category', title="", showgrid=False, autorange="reversed", tickfont=dict(size=14, weight='bold', color='#333'))
    ...
    with col_s6:
        ...
        # Displaying as clean stacked dashboard metrics
        st.metric("All-Time Favorite Riding Day", fav_day)
        st.metric("Longest Single Ride Ever", f"{longest_ride:,.1f} km", help=f"Achieved on {longest_ride_date}")
        st.metric("Total Lifetime Distance Logged", f"{lifetime_km:,.0f} km")
